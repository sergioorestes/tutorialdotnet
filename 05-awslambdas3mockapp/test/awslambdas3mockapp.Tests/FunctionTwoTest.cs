...
namespace awslambdas3mockapp.Tests;

public class FunctionTwoTest{
    [Fact]
    public async Task TestS3EventResponseLambdaFunction()
    {
        ...
        var function = new FunctionTwo(mockS3Client.Object);
        var response = await function.FunctionHandler(s3Event, testLambdaContext);

        Assert.Equal("text/plain", response);
    }
}