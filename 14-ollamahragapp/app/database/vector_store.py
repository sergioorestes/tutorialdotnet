...
class VectorStore:
    ...
    def __init__(cls):
        ...
        cls.embedding_model = "bert_tiny_embeddings_english_portuguese-unsloth:latest"
        cls.vector_settings = cls.settings.vector_store
        cls.base_url = cls.settings.mistral.base_url
        cls.vec_client = client.Sync(
            cls.settings.database.service_url,
            cls.vector_settings.table_name,
            cls.vector_settings.embedding_dimensions,
            time_partition_interval = cls.vector_settings.time_partition_interval,
        )

    @classmethod
    def get_embedding(cls, text: str) -> List[float]:
        """
        Generate embedding for the given text using Ollama (Mistral R1:14).
        ...
        try:
            ...
            embedding = ollama_embedding.get_query_embedding(text)[:4096]
        ...
        logging.info("Embedding generated in %.3f seconds", elapsed_time)
        return embedding

    @classmethod
    def create_tables(cls) -> None:
        """Create the necessary tablesin the database"""
        cls.vec_client.create_tables()

    @classmethod
    def create_index(cls) -> None:
        """Create the StreamingDiskANN index to spseed up similarity search"""
        cls.vec_client.create_embedding_index(client.DiskAnnIndex())

    @classmethod
    def drop_index(cls) -> None:
        """Drop the StreamingDiskANN index in the database"""
        cls.vec_client.drop_embedding_index()
    ...
    def upsert(cls, df: pd.DataFrame) -> None:
        ...
        cls.vec_client.upsert(list(records))
        logging.info(
            "Inserted %i records into %s", len(df), cls.vector_settings.table_name
        )
    ...
    def search(
        ...
        results = cls.vec_client.search(query_embedding, **search_args)
        elapsed_time = time.time() - start_time

        logging.info("Vector search completed in %.3f seconds", elapsed_time)

        if return_dataframe:
            return cls._create_dataframe_from_results(results)
        else:
            return results
    ...
    def delete(
        ...
        if delete_all:
            cls.vec_client.delete_all()
            logging.info(
                "Deleted all records from %s", cls.vector_settings.table_name
            )
        elif ids:
            cls.vec_client.delete_by_ids(ids)
            logging.info(
                "Deleted %i records from %s", len(ids), cls.vector_settings.table_name
            )
        elif metadata_filter:
            cls.vec_client.delete_by_metadata(metadata_filter)
            logging.info(
                "Deleted records matching metadata filter from %s", cls.vector_settings.table_name
            )
